(ns archival-curatorial.actor
  "ArchivalCuratorialActor — the ISCO-08 2621 independent archival-and-
  curatorial actor as a `langgraph.graph/state-graph` (per
  ADR-2607011000 / CLAUDE.md Actors section). One graph run = one
  collection operation request (intake → advise → govern → decide →
  ...
  The unconditional invariant: the CuratorialAdvisor can never
  directly commit a record or dispatch a robot action the
  ArchivalCuratorialGovernor refuses — every commit-record! call is
  gated behind `:decide`."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [archival-curatorial.advisor :as advisor]
            [archival-curatorial.governor :as governor]
            [archival-curatorial.store :as store]))

(defn build-graph
  "Build a compiled ArchivalCuratorialActor graph. `store` implements
  `archival-curatorial.store/Store`. `advisor` implements
  `archival-curatorial.advisor/Advisor` (defaults to `mock-advisor`).
  ...
  (-> (g/state-graph
      ...
      (g/add-node :commit
                   (fn [{:keys [request proposal]}]
                     (let [record {:item-id (:item-id request)
                                    :op (:op proposal)
                                    :payload proposal}]
